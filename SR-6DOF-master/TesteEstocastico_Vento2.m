...
Clalpha = calculaCoefSustentacao_mod(Geral, Coifa, Corpo, Aleta, MACH, alpha_rad);

% Coeficiente de Arrasto (Cd0 e Cdi)
Cd0 = calculaArrastoMinimo_mod(Geral, Coifa, Corpo, Aleta, Va, MACH);
Cdi = calculaArrastoAoA_mod(Geral, Coifa, Corpo, Aleta, Clalpha, alpha_rad);
...
while true
    ...
    % Coeficiente de Sustentação (Clalpha)
    Clalpha = calculaCoefSustentacao_mod(Geral, Coifa, Corpo, Aleta, MACH, alpha_rad)  + (1*randn);

    % Coeficiente de Arrasto (Cd0 e Cdi)
    Cd0 = calculaArrastoMinimo_mod(Geral, Coifa, Corpo, Aleta, Va, MACH) + (0.001*randn);
    Cdi = calculaArrastoAoA_mod(Geral, Coifa, Corpo, Aleta, Clalpha, alpha_rad) + (0.01*randn);
...
set(gcf,'color','w')
set(gcf,'Units','centimeters','Position',[8 8 25 14])
histogram2(Xfinal , Yfinal , 80 , 'normalization' , 'pdf' , 'FaceColor' , 'flat')
xlabel('x (m)')
ylabel('y (m)')
view(-38, 58)
colorbar
